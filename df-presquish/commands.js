...
const simulateLevel = (rioData, keyLevel) => {
	...
	for (const bestRun of mythic_plus_best_runs) {
		const dungeon = {
	...
	for (const altRun of mythic_plus_alternate_runs) {
		const dungeon = {
...
const getPushData = (rioData) => {
	...
	for (const run of mythic_plus_alternate_runs) {
		run.isBestRun = false;
		run.bestRating = sanitizeNumber(run.score * 1.5);
		run.altRating = sanitizeNumber(run.score / 2);
		push[run.short_name][run.affixes[0].name.toLowerCase()] = run;
	...
	for (const shortName of Object.keys(push)) {
...
const getGoalDungeonToRun = (dungeon, rioData, useRioUpgrades = false) => {
	...
	if (useRioUpgrades === true) {
		...
		const bestRioDungeon = rioData.mythic_plus_best_runs.find((run) => run.short_name === dungeon.shortName && run.mythic_level === dungeon.level)
...
const getBlankPushObject = () => {
	const result = {};
	for (const shortName of Object.keys(DUNGEON_SHORTNAME_MAP)) {
 ...
 */
const getCurrentWeekPrimaryAffix = () => axios.get("https://raider.io/api/v1/mythic-plus/affixes", { params: { region: "us", locale: "en" } })
